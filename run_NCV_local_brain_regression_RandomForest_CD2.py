...
if __name__ == "__main__":
    ...
    for fold in range(4):
        ...
        brainimage_shape = None
        for coordinate in coordinates:

            x, y = [], []
            for brainimage, label in train_set:
                brainimage = brainimage.squeeze().cpu()[coordinate[0]:coordinate[0]+20,
                                                        coordinate[1]:coordinate[1]+20,
                                                        coordinate[2]:coordinate[2]+20]
                brainimage_shape = brainimage.shape
                x.append(brainimage.flatten().numpy())
            ...
            for brainimage, label in test_set:
                x_test.append(brainimage.squeeze().cpu()[coordinate[0]:coordinate[0]+20,
                                                        coordinate[1]:coordinate[1]+20,
                                                        coordinate[2]:coordinate[2]+20].flatten().numpy())
            ...
            shap_values = shap_values.reshape(brainimage_shape).squeeze()
            print("Save SHAP values shape:",shap_values.shape) #

            # Save shap values
            os.makedirs(f'{args.SAVE_DIR_PATH}/fold{fold}/{args.SUBJECT}/{tuple(coordinate)}', exist_ok=True)
            np.save(f'{args.SAVE_DIR_PATH}/fold{fold}/{args.SUBJECT}/{tuple(coordinate)}/shap_values.npy', shap_values)
