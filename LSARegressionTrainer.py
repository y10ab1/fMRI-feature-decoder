...
class model_trainer:
    ...
    def create_data_loader(self, train_data, batch_size):
        ...
        def balance_prob(imbalanced_data):
            ...
            for _, genre in imbalanced_data:
                genres.append(train_data._getGenre(genre))
            genreCounter = Counter(genres)
            print(genreCounter)
            balance_prob = []
            for _, genre in imbalanced_data:
                balance_prob.append(1/genreCounter[train_data._getGenre(genre)])
            return balance_prob
    ...
    def train_single_epoch(self, model, data_loader, data_set = None, loss_fn = None, optimiser = None, device = 'cpu', mode = 'train', scheduler = []):
        ...
        for (BrainImage, InstrumentGenre) in loop:
            ...
            l1loss = torch.nn.L1Loss()(Prediction.float().squeeze(), InstrumentGenre.argmax(1).float()/24)
    ...
    def train(self, model, train_data_loader, valid_data_loader = None, data_set = None, loss_fn = None, optimiser = None, device = 'cpu', epochs = 100, scheduler = []):
        ...
        for i in range(epochs):
            ...
            if min_valid_loss > mean_epoch_val_loss:
                ...
                torch.save(model['Encoder'].state_dict(), f"{self.SAVE_DIR_PATH}/{self.MODEL_NAME}")
    ...
    def testing(self, model, data_loader, loss_fn = None, device = 'cpu'):
        ...
        for (BrainImage, InstrumentGenre) in loop:
            ...
            # explainer = shap.DeepExplainer(model['Encoder'], BrainImage)
            ...
            self.all_truth_log += (InstrumentGenre.argmax(1).float()/24).tolist()
            ...
            loop.set_postfix(test_loss = l1loss.item())

        

            epoch_loss.append(l1loss.item())
    ...
    def start_training(self):
        ...
        try:
            os.mkdir(f'{self.SAVE_DIR_PATH}', mode = 0o777, dir_fd = None)
        except FileExistsError:
            print(f'Directory {self.SAVE_DIR_PATH} exists.')


        try:
            encoder.load_state_dict(torch.load(f"{self.SAVE_DIR_PATH}/{self.MODEL_NAME}"))
        ...
        for _ in test_loop:
            ...
            spearcoef, spear_p_value = stats.spearmanr(self.all_pred_log, self.all_truth_log)

            self.test_loss += testloss
            self.corrcoef += coef
            self.p_value += p_value
            self.spearcoef += spearcoef
            self.spear_p_value += spear_p_value
