...
# For classify
        self.conv7 = nn.Sequential(
            nn.Conv1d(in_channels=128, out_channels=num_classes, kernel_size=1, padding=1),
        ...
        self.conv8 = nn.Sequential(
            nn.Conv1d(in_channels=128, out_channels=25, kernel_size=1, padding=1),
        ...
        #self.lazylinear_output = nn.LazyLinear(out_features=1)
...
###########################################################################
    ...
    def forward(self, input_data):
        ...
        # x = self.conv6(x)
        reg = self.conv7(x)
        reg = self.Hardtanh(reg)
        
        cla = self.conv8(x)
        cla = self.softmax(cla).squeeze()
        return reg, cla
...
if __name__ == "__main__":
    ...
    for p in encoder.parameters():
        print(p.shape)
    x = randn(1,1,240).cuda()
    y = encoder(x)
    print(y, y.shape)
    summary(encoder.cuda(), (1, 240))
