...
def find_target_voxels(args=None, fold:int=-1):
    ...
    topk_SB_coordinates = []
    # Load all minifolds shap values
    for mini_fold in range(4):
        SB_path = os.path.join(args.SB_DIR_PATH, f'fold{fold}', args.SUBJECT, f'minifold{mini_fold}-shap_brainImage.nii')
        SB = get_brainImage(SB_path)[:,:,10:-10]

        # Find topk SHAP value voxels' coordinates in high correlation cubes
        
        for coordinate in high_CB_coordinates:
            x, y, z = coordinate
            SB_cube = SB[x:x+args.LOCAL_BRAIN_SIDE_LENGTH, 
                        y:y+args.LOCAL_BRAIN_SIDE_LENGTH, 
                        z:z+args.LOCAL_BRAIN_SIDE_LENGTH]
    ...
    # Unique coordinates
    topk_SB_coordinates = list(set(topk_SB_coordinates))
